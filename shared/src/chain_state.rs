...
impl ChainState {
    ...
    pub fn resolve_tx_from_pending_and_proposed<'a, 'b>(
        &'a self,
        tx: &'b Transaction,
...
impl<'a> CellProvider<'a> for ChainState {
...
impl<'a, CS: ChainStore<'a>> CellProvider<'a> for ChainCellSetOverlay<'a, CS> {
    fn cell(&'a self, out_point: &OutPoint) -> CellStatus {
        if let Some(cell_out_point) = &out_point.cell {
...
impl<'a> BlockMedianTimeContext for &'a ChainState {
